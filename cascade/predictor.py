...
class Predictor:
    def __init__(self):
        C_modelpath = os.path.join(os.path.dirname(__file__), 'best_model_C.hdf5')
        self.C_model = load_model(C_modelpath, custom_objects={'GraphModel': GraphModel,
                                                    'ReduceAtomToPro': ReduceAtomToPro,
                                                    'Squeeze': Squeeze,
                                                    'GatherAtomToBond': GatherAtomToBond,
                                                    'ReduceBondToAtom': ReduceBondToAtom})
        H_modelpath = os.path.join(os.path.dirname(__file__), 'best_model_H.hdf5')
        self.H_model = load_model(H_modelpath, custom_objects={'GraphModel': GraphModel,
                                                    'ReduceAtomToPro': ReduceAtomToPro,
                                                    'Squeeze': Squeeze,
                                                    'GatherAtomToBond': GatherAtomToBond,
                                                    'ReduceBondToAtom': ReduceBondToAtom})

        with open(os.path.join(os.path.dirname(__file__), 'preprocessor.p'), 'rb') as ft:
            self.preprocessor = pickle.load(ft)['preprocessor']

    def _Mol_iter(self, df):
        for index,r in df.iterrows():
            yield(r['Mol'], r['atom_index'])

    def _preprocess(self, type):

        df = []
        for m in self.mols_conf:
            if m:
                E = float(m.GetProp('E'))
                m_id, cf_id = [ int(x) for x in m.GetProp('_Name').split('_') ]
                As = [x for x in m.GetAtoms() if x.GetAtomicNum()==[6,1][type == 'H']]
                A_index = np.array([x.GetIdx() for x in As]).astype(int)
                df.append([m_id, m, m.GetNumAtoms(), A_index, E, cf_id])

        df = pd.DataFrame(df, columns=['mol_id', 'Mol', 'n_atoms', 'atom_index', 'relative_E', 'cf_id'])

        inputs = self.preprocessor.predict(self._Mol_iter(df))
    ...
    def _evaluate(self, inputs, type='C'):
        assert type == 'C' or type =='H'
        batch_size = 32
        evaluate_sequence = Predictor.RBFSequence(inputs, batch_size=batch_size)

        predicted = []
    
        model = [self.C_model, self.H_model][type == 'H']
    ...
    def genConf(self, smiles, keep_all_cf=True):
        self.mols = [Chem.MolFromSmiles(smiles)]
        for m in self.mols: AllChem.EmbedMolecule(m, useRandomCoords=True)
        self.mols = [Chem.AddHs(m, addCoords=True) for m in self.mols]

        self.mols_conf = []
        for i,m in enumerate(self.mols):
            try:
                mol,ids,nr = genConf.genConf(m, rms=-1, nc=200, efilter=10.0, rmspost=0.5)
            except Exception as e:
                msg = "cannot generate FF conformers"
                print(msg)
                self.mols_conf.append(None)
                continue
            ...
            if keep_all_cf:
                ...
                self.mols_conf.extend(mols_i)
            else:
                id = ids[0]
                m.SetProp('E', str(id[0]))
                m.SetProp('_Name', '{}_{}'.format(i, id[1]))
                self.mols_conf = [Chem.RWMol(m)]
        return

    def predict_NMR(self, type='C'):
        assert type == 'C' or type =='H'

        inputs, df = self._preprocess(type)
        predicted = self._evaluate(inputs,type)
        ...
        for (m_id, a_id),df in df_group:
            ...
            final.append([a_id, weighted_shift])

        final = pd.DataFrame(final, columns=['atom_index', 'Shift'])
